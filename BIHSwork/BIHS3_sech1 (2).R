...
BIHS3_hhsecH1_paddytype_season %>% 
  ...
  pivot_wider(id_cols = hhid,
              ...
              values_from = adopt,
              values_fill = 0) %>% 
  mutate(counthh = 1) %>% ### this is for counting hhds 
  colSums()
...
BIHS3_Adopt <- 
  ...
  pivot_wider(id_cols = hhid,
              ...
              values_from = adopt,
              values_fill = 0) %>%
  rename(Adopt_HYV = HYV,
         Adopt_Hybrid = Hybrid, 
         Adopt_Local = `Local Variety`)

BIHS3_paddytypearea_season_wide <-
  left_join(BIHS3_paddytypearea_season_wide, BIHS3_Adopt)
