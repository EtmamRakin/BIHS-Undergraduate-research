...
BIHS3_dfest_temp <-
  BIHS3_dfest %>% select(hhid, fhh, hh_age, hhsize, div_name,
                         totalasset, hh_educ, literacy_status, BRRIdhan, Bina,
...
BIHS3_dfest_temp %>% 
  ...
  tbl_summary(
    ...
    label = list(fhh = "Female headed houshold",
                 ...
                 literacy_status = "Literacy (based on reading/writing capabilties)",
...
est_area_Boro_Hybrid_Barisal <-
  lm(share_Boro_Hybrid ~ log(area_Boro_total) + hh_educ + literacy_status + hh_age + hhsize + I(Female_1564/hhsize) + I(Male_1564/hhsize) + I(Female_65ov/hhsize) + I(Male_65ov/hhsize) + 
       fhh + totalasset + 
       advice_irri + advice_seed + 
       BRRIdhan + Bina +
       agrcultrl_fish_lstock_groupmem, data = BIHS3_dfest_Boro %>% filter(div_name == "Barisal")) 
...
#summary statistics for aman aus boro rice facrmers ---tables
